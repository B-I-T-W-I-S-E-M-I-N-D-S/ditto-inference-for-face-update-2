...
class AvatarRegistrar:
    ...
    def __init__(
        ...
        """
        Args:
            face_preprocessor  : optional FacePreprocessor — fed to Source2Info
                                 so AppearanceExtractor receives a cleaned image.
            canonical_template : optional CanonicalTemplate — overrides per-frame
                                 x_s_info geometry with template-blended values.
        """
    ...
    def register(
        ...
        **kwargs,
        ...
        for rgb in rgb_list:
            ...
            # ── Canonical geometry override ───────────────────────────────
            if self.canonical_template is not None:
                info["x_s_info"] = self.canonical_template.blend_x_s_info(
                    info["x_s_info"]
                )
            # ─────────────────────────────────────────────────────────────
        ...
        # ── Canonical sc (source-condition) blend ─────────────────────────
        # sc feeds the DiT condition; blend it to match overridden geometry.
        if self.canonical_template is not None:
            sc_f0 = self.canonical_template.blend_sc(sc_f0)
        # ─────────────────────────────────────────────────────────────────
